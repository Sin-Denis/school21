...
/*   Updated: 2019/08/17 15:38:42 by jblue-da         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			output_body(t_graph *g,
t_vector **roads, t_vector *distribution, int num_roads)
{
	int				i;
	int				j;
	int				num_lines;
	...
	while (++i < num_lines)
	{
		...
		while (++j < num_roads)
			...
			get_distr_offset(distribution->data[j],
			vector_get_elem(offsets, j)));
		ft_printf("\n");
	}
