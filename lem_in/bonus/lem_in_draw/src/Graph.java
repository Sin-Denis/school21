...
public class Graph extends JComponent {
   ...
   ArrayList<Ant_draw> ants;
   ArrayList<Pair<Integer, Integer>> transitions;
   ArrayList<Pair<Integer, Integer>> state;
   ...
   Graph() {
      ...
      this.transitions = new ArrayList<Pair<Integer, Integer>>();
      this.state = new ArrayList<Pair<Integer, Integer>>();
      this.ants = new ArrayList<Ant_draw>();
...
//         vertex.printAdj();
   ...
   public void paintComponent(Graphics g) {
      ...
      while(!queue.isEmpty()) {
         ...
         for (Integer u_idx : v.adj) {
            ...
            if (v_idx == start_idx && u_idx == end_idx)
               g2.drawLine(v.x + 10, v.y + 10, u.x + 10, u.y + 10);
            else if (v_idx == start_idx || v_idx == end_idx)
               g2.drawLine(v.x + 10, v.y + 10, u.x + 5, u.y + 5);
            else if (u_idx == end_idx || u_idx == start_idx)
               g2.drawLine(v.x + 5, v.y + 5, u.x + 10, u.y + 10);
            else
               g2.drawLine(v.x + 5, v.y + 5, u.x + 5, u.y + 5);
            if (visited.get(u_idx) != 0) {
               continue;
            }
            visited.set(u_idx, 1);
   ...
   public void initState() {
      for (int i = 0; i < this.num_ants; ++i) {
         this.state.add(new Pair<Integer, Integer>(i, this.start_idx));
      }
   }

   public void initAnts() {
      Vertex startRoom = this.getVertex(this.start_idx);
      for (int i = 0; i < this.num_ants; ++i) {
         this.ants.add(new Ant_draw(startRoom.x + startRoom.x / 4, startRoom.y + startRoom.y / 4));
      }
   }
